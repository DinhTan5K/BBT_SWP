...
[Authorize(AuthenticationSchemes = "BranchManagerScheme")]
...
public class BManagerController : Controller
{
    ...
    /// <summary>
    /// Lấy BranchID từ session hoặc database (fallback)
    /// </summary>
        ...
        if (!branchId.HasValue)
        {
            ...
            if (!string.IsNullOrEmpty(empId))
            {
                ...
                if (employee != null && employee.BranchID.HasValue)
                {
                    ...
                    // Cập nhật session để lần sau không cần query lại
    ...
    public async Task<IActionResult> Index()
    {
        ...
        // Lấy thống kê nhân viên hàng tuần và gán vào ViewBag
    ...
    public async Task<IActionResult> ViewEmp()
    {
        ...
        // Sửa lại để truy vấn trực tiếp và bao gồm cả Shipper (SP)
        var rolesToView = new[] { "EM", "SL", "SP" };
        var employees = await _context.Employees
            .Where(e => e.BranchID == CurrentBranchId.Value && rolesToView.Contains(e.RoleID))
            .ToListAsync();
...
{
    System.Diagnostics.Debug.WriteLine("🔥 CreateEmpPartial CALLED!");

    var roles = await _employeeService.GetSelectableRolesAsync();

    System.Diagnostics.Debug.WriteLine("🔥 ROLES COUNT = " + roles.Count());
    foreach (var r in roles)
    {
        System.Diagnostics.Debug.WriteLine($"➡ ROLE: {r.RoleID} - {r.RoleName}");
    }
...
public async Task<IActionResult> CreateEmp(EmployeeBranchRequest request) 
{
    ...
    if (ModelState.IsValid)
    {
        ...
        if (success)
        {
            // Trả về JSON để JavaScript xử lý, thay vì trả về PartialView
...
public async Task<IActionResult> GetEmployeeShiftsPartial(string employeeId, int month, int year)
{
    ...
    public async Task<IActionResult> RefundRequests()
    {
        ...
        if (!branchId.HasValue)
        {
            TempData["Error"] = "Bạn chưa được gán chi nhánh. Vui lòng liên hệ quản trị viên.";
            return RedirectToAction("Dashboard");
        }

        // Lọc orders theo BranchID và status "Chờ hoàn tiền"
        var pendingRefunds = await _context.Orders
            .Include(o => o.Customer)
            .Where(o => o.Status == "Chờ hoàn tiền" && o.BranchID == branchId.Value)
    ...
    public async Task<IActionResult> ApproveRefund(int orderId)
    {
        ...
        {
            TempData["Error"] = $"Đơn hàng {order.OrderCode} không ở trạng thái 'Chờ hoàn tiền'. Trạng thái hiện tại: {order.Status}";
            return RedirectToAction(nameof(RefundRequests));
        }

        // Kiểm tra TransId - Nếu không có TransId thì vẫn có thể hoàn tiền (test mode)
        bool hasTransId = !string.IsNullOrEmpty(order.TransId);
        ...
        string? refundTransId = null;

        if (hasTransId)
        {
            // Có TransId - Gọi MoMo API (hoặc mock trong test mode)
            try
            {
                string momoResponseJson = await _paymentService.RefundAsync(
                    order.TransId!, 
                    order.Total, // Hoàn tiền toàn bộ
                    description
                );
                ...
                if (root.TryGetProperty("resultCode", out var resultCodeElement) && resultCodeElement.GetInt32() == 0)
                {
                    ...
                    // Lấy refund transaction ID nếu có
                    if (root.TryGetProperty("orderId", out var orderIdElement))
                    {
                        refundTransId = orderIdElement.GetString();
                    }
                ...
                else
                {
                    ...
                    {
                        momoResultMessage = $"Hoàn tiền thất bại. Lỗi: {messageElement.GetString()}";
                    }
                    else
                    {
                        momoResultMessage = "Hoàn tiền thất bại. Không thể xử lý yêu cầu.";
                    }
                }
            }
            catch (JsonException ex)
            {
                momoResultMessage = $"Lỗi phân tích phản hồi JSON: {ex.Message}";
            }
            catch (Exception ex)
            {
                momoResultMessage = $"Lỗi khi xử lý hoàn tiền: {ex.Message}";
            }
        }
        else
        {
            // Không có TransId - Chỉ update status (test mode hoặc đơn hàng chưa thanh toán qua MoMo)
            refundSuccess = true;
            refundTransId = $"MANUAL_{Guid.NewGuid():N}";
            momoResultMessage = $"Đã hoàn tiền thủ công cho đơn hàng {order.OrderCode} (không có TransId).";
        }
        ...
        if (refundSuccess)
        {
            ...
            if (!string.IsNullOrEmpty(refundTransId))
            {
                order.RefundTransId = refundTransId;
            }
    ...
    public async Task<IActionResult> GetOrderDetailsForRefund(int orderId)
    {
        ...
        if (string.IsNullOrEmpty(empId))
        {
            return NotFound("Phiên đăng nhập hết hạn. Vui lòng đăng nhập lại.");
        }
        ...
        if (!branchId.HasValue)
        {
            return NotFound("Bạn chưa được gán chi nhánh. Vui lòng liên hệ quản trị viên.");
        }
        ...
        if (order == null) return NotFound("Đơn hàng không tồn tại hoặc không thuộc chi nhánh của bạn.");
