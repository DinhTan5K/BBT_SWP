...
using start.DTOs.Employee;
namespace start.Controllers
{

    [Route("Employee")] // /Employee/...
    [Authorize(AuthenticationSchemes = "EmployeeScheme,ShiftLeaderScheme,ShipperScheme,MarketingScheme")]
    public class EmployeeController : Controller
    {
        ...
        // SỬA LẠI: Cho phép cả Shipper (SP, SP) truy cập
        // CHO PHÉP: NV | EM | SL | SP | SP
        private bool CanAccessDayOff() {
            var normalizedRole = CurrentRole?.Trim().Replace(" ", "").ToUpperInvariant();
            return normalizedRole is "NV" or "EM" or "SL"  or "SP";
        }
        ...
        public IActionResult Profile()
        {
            ...
            // Đồng bộ email vào session nếu có giá trị và session chưa có
            var sessionEmail = HttpContext.Session.GetString("Email");
            if (string.IsNullOrWhiteSpace(sessionEmail) && !string.IsNullOrWhiteSpace(emp.Email))
            {
                HttpContext.Session.SetString("Email", emp.Email);
            }
            else if (string.IsNullOrWhiteSpace(emp.Email))
            {
                HttpContext.Session.Remove("Email");
            }
            
            // Truyền role vào ViewBag để view có thể check và dùng layout phù hợp
        ...
        public async Task<IActionResult> UploadAvatar(IFormFile? avatar)
        {
            ...
            {
                TempData["err"] = "File ảnh không hợp lệ hoặc file trống.";
                return RedirectToAction(nameof(EditProfile));
            }
            ...
            {
                TempData["err"] = $"File ảnh quá lớn. Kích thước tối đa là {maxFileSize / (1024 * 1024)}MB.";
                return RedirectToAction(nameof(EditProfile));
            }
            ...
            {
                TempData["err"] = $"Định dạng file không hợp lệ. Chỉ chấp nhận: {string.Join(", ", allowedExtensions)}";
                return RedirectToAction(nameof(EditProfile));
            }
            ...
            try
            {
                ...
                if (!ok) 
                {
                    TempData["err"] = "Upload ảnh thất bại. Vui lòng kiểm tra lại file ảnh hoặc thử lại sau.";
                }
                else 
                {
                    TempData["ok"] = "Đã cập nhật ảnh đại diện thành công.";
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[UploadAvatar Controller] Exception: {ex.Message}");
                TempData["err"] = $"Lỗi khi upload ảnh: {ex.Message}";
            }
        ...
        public IActionResult Chat(string? id)
        {
            ...
            ViewBag.CurrentRole = CurrentRole; // Truyền role để view check layout
            return View(emp);
        }
        [HttpGet("Contract/{id}")]
        public IActionResult Contract(string id)
        {
            ...
            ViewBag.ActiveMenu = "Profile";
            ViewBag.CurrentRole = CurrentRole; // Truyền role để view check layout
            ViewBag.Employee = contract.Employee; // Truyền employee để view có thể check role từ employee
            ViewData["Employee"] = contract.Employee;
        ...
        public async Task<IActionResult> CheckIn(int? workScheduleId)
        {
            ...
            if (string.IsNullOrEmpty(empId))
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Vui lòng đăng nhập.", IsCheckIn = true, WorkScheduleId = workScheduleId });


            var emp = await _db.Employees.FindAsync(empId);
            if (emp == null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Không tìm thấy nhân viên.", IsCheckIn = true, WorkScheduleId = workScheduleId });

            if (string.IsNullOrEmpty(emp.AvatarUrl))
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Bạn chưa có ảnh đại diện để nhận diện khuôn mặt. Vui lòng cập nhật trong Edit Profile.", IsCheckIn = true, WorkScheduleId = workScheduleId });
            ...
            if (schedule == null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = $"Hôm nay ({today:dd/MM/yyyy}) bạn không có ca làm việc.", IsCheckIn = true, WorkScheduleId = workScheduleId });

            // 🔽🔽🔽 Chính là 2 đoạn bạn hỏi ở đây 🔽🔽🔽
            var now = DateTime.Now;
            if (!ShiftTimeHelper.CanCheckIn(now, schedule.Date, schedule.Shift, out var msg))
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = msg, IsCheckIn = true, WorkScheduleId = schedule.WorkScheduleID });

            var already = await _attendanceService.GetTodayCheckInAsync(empId);
            if (already != null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Bạn đã check-in hôm nay. Vui lòng check-out trước.", IsCheckIn = true, WorkScheduleId = schedule.WorkScheduleID });
            // 🔼🔼🔼 Hết 2 đoạn kiểm tra này 🔼🔼🔼

            // ✅ Nếu qua được hết mấy bước trên thì render modal có video
            return PartialView("_CheckInModal", new CheckInModalViewModel
            {
                CanStart = true,
                Message = "",
                IsCheckIn = true,
        ...
        public async Task<IActionResult> CheckOut(int? workScheduleId)
        {
            var empId = CurrentEmpId;
            if (string.IsNullOrEmpty(empId))
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Vui lòng đăng nhập.", IsCheckIn = false, WorkScheduleId = workScheduleId });

            var checkIn = await _attendanceService.GetTodayCheckInAsync(empId);
            if (checkIn == null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Bạn chưa check-in hôm nay.", IsCheckIn = false, WorkScheduleId = workScheduleId });

            if (checkIn.CheckOutTime != null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Bạn đã check-out rồi.", IsCheckIn = false, WorkScheduleId = workScheduleId });

            // ✅ Lấy workScheduleId và schedule để kiểm tra giờ check-out
            WorkSchedule? schedule = null;
            if (workScheduleId.HasValue)
            {
                schedule = await _db.WorkSchedules
            ...
            {
                schedule = await _db.WorkSchedules
                    .FirstOrDefaultAsync(w => w.EmployeeID == empId && w.Date.Date == DateTime.Today);
            }

            if (schedule == null)
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = "Không tìm thấy ca làm việc hôm nay.", IsCheckIn = false, WorkScheduleId = workScheduleId });

            // ✅ Kiểm tra giờ check-out
            var now = DateTime.Now;
            if (!ShiftTimeHelper.CanCheckOut(now, schedule.Date, schedule.Shift, out var msg))
                return PartialView("_CheckInModal", new CheckInModalViewModel { CanStart = false, Message = msg, IsCheckIn = false, WorkScheduleId = schedule.WorkScheduleID });
        ...
        public class CheckInRequest
        {
            ...
            public IActionResult CreateNewsRequest()
            {
                ...
                var activeDiscounts = _db.Discounts
                    .Where(d => d.IsActive && (d.EndAt == null || d.EndAt > DateTime.Now))
